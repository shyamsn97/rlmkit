...
|---|---|
| `01_query.py` | flat views (`graph.nodes`, `.agents`, `.edges`), filters (`.where`, `.queries()`, action views, `.errors()`), `find()`, `tokens()`, `result()` |
...
| `05_replay.py` | `retrace_steps(graph)` — reconstruct intermediate snapshots from the final graph |
